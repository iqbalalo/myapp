...
class ReadRequest(BaseModel):
    ...
    limit: Optional[int] = Field(default=100, ge=1, le=10000, description="Max records to return")
    offset: Optional[int] = Field(default=0, ge=0, description="Number of records to skip")
    include_fields: Optional[List[str]] = Field(default=None, description="Specific fields to include")
    ...
    sort_dir: Optional[str] = Field(default="ASC", regex="^(ASC|DESC|asc|desc)$", description="Sort direction")
...
class CreateRequest(BaseModel):
    ...
    schema_name: str = Field(..., description="Database schema name")
    table_name: str = Field(..., description="Table name")
    data: Union[Dict[str, Any], List[Dict[str, Any]]] = Field(..., description="Data to insert (single record or list)")
    hash_password: Optional[bool] = Field(default=False, description="Whether to hash password field")
    salt_field: Optional[str] = Field(default="email", description="Field to use as salt for password hashing (email or phone)")
...
class UpdateRequest(BaseModel):
    ...
    hash_password: Optional[bool] = Field(default=False, description="Whether to hash password field")
    salt_field: Optional[str] = Field(default="email", description="Field to use as salt for password hashing")
...
class JoinReadRequest(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(default={}, description="Filter conditions")
    select_fields: Optional[List[str]] = Field(default=None, description="Specific fields to select (use table.column format)")
    limit: Optional[int] = Field(default=100, ge=1, le=10000)
    offset: Optional[int] = Field(default=0, ge=0)
    sort_by: Optional[str] = Field(default=None, description="Column to sort by (use table.column format)")
    sort_dir: Optional[str] = Field(default="ASC", regex="^(ASC|DESC|asc|desc)$")
...
class RawQueryRequest(BaseModel):
    ...
    params: Optional[List[Any]] = Field(default=None, description="Query parameters for prepared statements")
    read_only: Optional[bool] = Field(default=True, description="Whether query is read-only (SELECT)")
...
class PasswordVerifyRequest(BaseModel):
    ...
    schema_name: str = Field(..., description="Database schema name")
    table_name: str = Field(..., description="Table name")
    identifier_field: str = Field(..., description="Field to identify user (email or phone)")
    identifier_value: str = Field(..., description="Value of identifier field")
    password: str = Field(..., description="Password to verify")
    salt_field: Optional[str] = Field(default=None, description="Field used as salt (defaults to identifier_field)")
...
class BulkOperationRequest(BaseModel):
    ...
    schema_name: str = Field(..., description="Database schema name")
    table_name: str = Field(..., description="Table name")
    operation: str = Field(..., regex="^(create|update|delete)$", description="Operation type")
...
def encrypt_password_in_data(data: Dict[str, Any], salt_field: str = "email") -> Dict[str, Any]:
    """Encrypt password field in data dictionary"""
    if "password" in data and data["password"]:
        if salt_field not in data or not data[salt_field]:
            raise ValueError(f"Salt field '{salt_field}' is required for password hashing")
    ...
    try:
        ...
        if request.include_fields or request.exclude_fields:
            ...
            if request.include_fields:
                final_columns = [col for col in request.include_fields if col in all_columns]
...
async def update_records(request: UpdateRequest):
    ...
    try:
        ...
        if request.hash_password and "password" in update_data:
            ...
            where_clause, where_values = build_where_clause(request.filters)
            salt_query = f'SELECT "{request.salt_field}" FROM {full_table_name}' + where_clause + " LIMIT 1"
            cursor.execute(salt_query, where_values)
            salt_record = cursor.fetchone()
            
            if salt_record:
                salt_value = salt_record[request.salt_field]
                update_data["password"] = hash_password(update_data["password"], str(salt_value))
...
async def bulk_operation(request: BulkOperationRequest):
    ...
    try:
        ...
        if request.operation == "create":
            ...
            for record in request.records:
                ...
                if request.hash_password and "password" in data:
                    ...
                    salt_query = f'SELECT "email" FROM {full_table_name}' + where_clause + " LIMIT 1"
                    cursor.execute(salt_query, where_values)
                    salt_record = cursor.fetchone()
                    if salt_record:
                        data["password"] = hash_password(data["password"], salt_record["email"])
        ...
        return {
            ...
            "message": f"{len(results)} records processed" if len(results) > 100 else None
        }
